...
(function () {
    ...
    function initDiveChooser() {
        ...
        readBtn.addEventListener('click', () => {
            ...
            const desc = readBtn.querySelector('.dive-option-desc');
            if (desc) {
                desc.style.transition = 'opacity 0.3s ease';
                desc.style.opacity = '0';
                setTimeout(() => {
                    desc.textContent = '✓ You\'re all set — scroll down to start exploring!';
    ...
    function initAudioPlayer() {
        ...
        audio.addEventListener('timeupdate', () => {
            if (!isSeeking) {
                ...
                seekSlider.style.background = `linear-gradient(to right, var(--surface) ${pct}%, var(--white-10) ${pct}%)`;
            }
        });
        ...
        seekSlider.addEventListener('input', () => {
            ...
            const pct = (seekSlider.value / audio.duration) * 100 || 0;
            seekSlider.style.background = `linear-gradient(to right, var(--surface) ${pct}%, var(--white-10) ${pct}%)`;
        });
    ...
    function showAudioPlayer() {
        ...
        setTimeout(() => {
            audio.play().catch(() => { /* auto-play blocked, user can press play */ });
        }, 600);
    }
    ...
    function initBreadcrumbs() {
        const CARD_BREADCRUMBS = {
            ...
            19: { section: 'Difficult to Prevent', chapter: 4, index: 1, total: 5 },
            21: { section: 'Difficult to Prevent', chapter: 4, index: 2, total: 5 },
            24: { section: 'Difficult to Prevent', chapter: 4, index: 3, total: 5 },
            23: { section: 'Difficult to Prevent', chapter: 4, index: 4, total: 5 },
            25: { section: 'Difficult to Prevent', chapter: 4, index: 5, total: 5 },
            27: { section: 'Mindset Shifts', chapter: 5, index: 1, total: 5 },
            29: { section: 'Mindset Shifts', chapter: 5, index: 2, total: 5 },
            30: { section: 'Mindset Shifts', chapter: 5, index: 3, total: 5 },
            31: { section: 'Mindset Shifts', chapter: 5, index: 4, total: 5 },
            32: { section: 'Mindset Shifts', chapter: 5, index: 5, total: 5 },
        };
